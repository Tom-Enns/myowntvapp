...
class TestFindStreamInHtml:
    def test_atob_pattern(self, backend):
        encoded = b64encode(b"https://stream.example.com/live/master.m3u8").decode()
        html = f"var url = atob('{encoded}');"
        assert backend._find_stream_in_html(html) == "https://stream.example.com/live/master.m3u8"

    def test_source_pattern(self, backend):
        html = "source: 'https://cdn.example.com/hls/live/stream.m3u8',"
        assert backend._find_stream_in_html(html) == "https://cdn.example.com/hls/live/stream.m3u8"

    def test_m3u8_url_pattern(self, backend):
        html = """file: 'https://live.example.com/hls/game.m3u8?token=abc'"""
        assert backend._find_stream_in_html(html) == "https://live.example.com/hls/game.m3u8?token=abc"

    def test_playlist_load_pattern(self, backend):
        html = """var url = 'https://cdn.example.com/playlist/123/load/stream';"""
        assert backend._find_stream_in_html(html) == "https://cdn.example.com/playlist/123/load/stream"

    def test_playlist_path_pattern(self, backend):
        html = """var url = 'https://cdn.example.com/live/playlist/stream.m3u8';"""
        assert backend._find_stream_in_html(html) == "https://cdn.example.com/live/playlist/stream.m3u8"

    def test_no_stream_found(self, backend):
        html = "<html><body>No streams here</body></html>"
        assert backend._find_stream_in_html(html) is None

    def test_atob_from_fixture(self, backend):
        html = load_fixture("iframe_with_atob_stream.html")
        result = backend._find_stream_in_html(html)
        assert result == "https://stream.example.com/live/master.m3u8"

    def test_source_from_fixture(self, backend):
        html = load_fixture("iframe_with_source_stream.html")
        result = backend._find_stream_in_html(html)
        assert result == "https://cdn.example.com/hls/live/stream.m3u8"

    def test_m3u8_from_fixture(self, backend):
        html = load_fixture("iframe_with_m3u8_url.html")
        result = backend._find_stream_in_html(html)
        assert result == "https://live.example.com/hls/game123/playlist.m3u8?token=abc123"

    def test_no_stream_from_fixture(self, backend):
...
class TestExtractStream:
    @pytest.mark.asyncio
    async def test_tv_channel_extraction(self, backend):
        """TV channel page → token endpoint → stream URL."""
        ...
        with aioresponses() as m:
            ...
            stream = await backend._extract_stream("https://thetvapp.to/tv/espn/")
    ...
    async def test_iframe_extraction_atob(self, backend):
        ...
        with aioresponses() as m:
            ...
            stream = await backend._extract_stream("https://thetvapp.to/event/test/")
    ...
    @pytest.mark.asyncio
    async def test_iframe_no_stream_raises(self, backend):
        """Event page with iframe but no stream URL should raise."""
        event_page = load_fixture("thetvapp_event_page_iframe.html")
        ...
        with aioresponses() as m:
            ...
            with pytest.raises(RuntimeError, match="No HLS stream found"):
